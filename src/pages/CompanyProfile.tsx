  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      const { data } = await supabase
        .from("profiles")
        .select("business_name, business_cnpj, business_address, business_phone, business_logo_url")
        .eq("id", user.id)
        .single();
      if (data) {
        setForm({
        ...
        <CardContent>
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="business_cnpj"
                value={form.business_cnpj}
                onChange={(e) => setForm({ ...form, business_cnpj: e.target.value })}
